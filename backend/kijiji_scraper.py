...
KIJIJI_URLS = [
    "https://www.kijiji.ca/b-motorcycles/ontario/c30l9004?view=list"
]
...
async def process_page(context, url, cutoff):
    ...
    for link_tag in links:
        ...
        try:
            ...
            seen_listings[link] = listing_obj["timestamp"]
        ...
        if all_new:
            try:
                ...
                with open(EXPORT_JSON_PATH, "w") as f:
                    json.dump(list(all_new.values()), f, indent=2)
                print(f"💾 Exported {len(all_new)} new listings to {EXPORT_JSON_PATH}")
            except Exception as e:
                print(f"❌ Failed to write export JSON: {e}")

        seen_listings = {
            link: ts for link, ts in seen_listings.items()
